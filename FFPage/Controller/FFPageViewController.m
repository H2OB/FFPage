...
- (void)setUpRunloop{
    ...
    NSTimer * timer = [NSTimer timerWithTimeInterval:.1 target:self selector:@selector(timerChanged) userInfo:nil repeats:YES];
...
- (void)updateDisplay{
    ...
    //没有当前控制器代表没有初始化 设置当前页是默认页
    if(!_currentController) _currentPage = self.defaultPage;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(pageViewController:contentOffSetChanged:)]){
        [self.delegate pageViewController:self contentOffSetChanged:scrollView.contentOffset];
    }
